...
Il y a significativement un rapport au seuil alpha = 5% entre le taux de consommation d'eau quotidienne et l'indice de masse corporelle. (X²obs = 9.77 ; Ddl = 3; Valeur de p = 0.0206)
...
H0 : Il y a dépendance entre le tour du poignet et le pourcnetage de masse grass.
...
Nous ne pouvons pas dire que la durée de sommeil change de manière significative le logarithme de l'indice de rondeur corporelle (test t de Welch indépendant, ddl = 107.74, p = 0.06)
...
On ne rejette pas H0 au seuil alpha de 5% (t = 0.58851, df = 141 , p-value = 0.5571). C'est à dire que la moyenne du bmi pour les individus mangeant entre 0 et 2 fast-food par mois est égale à la moyenne du bmi des individus mangeant plus de 3 fast-food par mois.
...
#test ANOVA
biometry_anova<- lm(data = biometry_E, IRC ~ sucre)
anova(biometry_anova) |>tabularise()
#vérification homoscédasticité 
bartlett.test(data = biometry_E, IRC ~sucre)
#vérification normalité
chart$qqplot(biometry_anova, lang = "fr")
#test post-hoc
biometry_posthoc<- confint(multcomp::glht(biometry_anova,
...
```{r plot_anova1_2}
chart(data=biometry, tour_taille~alimentation)+
  geom_boxplot()
