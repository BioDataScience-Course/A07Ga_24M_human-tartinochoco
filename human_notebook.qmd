...
biometry$fumeur_rec_yes_no <- cut(biometry$fumeur,
  include.lowest = TRUE,
  right = FALSE,
  dig.lab = 4,
  breaks = c(-100, 0.1, 1000),
  labels=c("Non-fumeur", "Fumeur")
)
label(biometry$fumeur_rec_yes_no)<- "Comportement lors de la mesure"
units(biometry$fumeur_rec_yes_no) <- ""
biometry%>.%
  sselect(., fumeur, fumeur_rec_yes_no)->
  verif_fumeur
#skimr::skim(verif_fumeur)

chart(data= biometry, pmg~fumeur_rec_yes_no)+
...
chart(data= biometry, pmg~fumeur_rec_yes_no)+
  geom_jitter(width=0.3, alpha=0.2)+
  stat_summary(geom="point", fun="mean")+
  stat_summary(geom="errorbar", width=0.3, fun.data="mean_sdl", fun.args = list(mult=1))
```
...
```{r test_t_1}
car::qqPlot(biometry$fumeur, distribution = "norm",
...
units(biometry$fast_food_rec_yes_no) <- "nb/mois"
biometry%>.%
