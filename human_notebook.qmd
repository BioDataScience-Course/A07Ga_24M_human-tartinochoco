...
```{r test_t_1}
#test
t.test(data= biometry_E, log_IRC~sommeil_rec , alternative="two.sided", conf.level= 0.95, var.equal = TRUE)
```
...
<!--% Interprétation statistique et biologique --> On ne rejette pas H0 au seuil alpha de 5% (t = 0.58851, df = 141 , p-value = 0.5571). C'est à dire que la moyenne du bmi pour les individus mangeant entre 0 et 2 fast-food par mois est égale à la moyenne du bmi des individus mangeant plus de 3 fast-food par mois.
